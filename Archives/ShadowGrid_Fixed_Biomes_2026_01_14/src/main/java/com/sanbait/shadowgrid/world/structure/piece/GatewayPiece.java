...
public class GatewayPiece extends StructurePiece {
    ...
    public void postProcess(WorldGenLevel level, StructureManager structureManager, ChunkGenerator generator,
        ...
        // Если это вертикальная стена (mod 15) -> сдвигаем вправо на x+13 (Вплотную к
        // 16)
        if (modX == 15) {
            startX = chunkMinX + 13;
            startZ = chunkMinZ + 7;
        }
        // Если это ВНЕШНЯЯ сторона (mod 16) -> сдвигаем влево на x+0 (Вплотную к 0)
        else if (modX == 16) {
            startX = chunkMinX + 0;
            startZ = chunkMinZ + 7;
        }

        // Если это горизонтальная стена (mod 15) -> сдвигаем вниз на z+13
        else if (modZ == 15) {
            startX = chunkMinX + 7;
            startZ = chunkMinZ + 13;
        }
        // Внешняя сторона (mod 16) -> сдвигаем вверх на z+0
        else if (modZ == 16) {
            startX = chunkMinX + 7;
            startZ = chunkMinZ + 0;
        }

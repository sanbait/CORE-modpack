        ...
        public static void onClientTick(net.minecraftforge.event.TickEvent.ClientTickEvent event) {
            if (event.phase == net.minecraftforge.event.TickEvent.Phase.END && NexusCore.RENDER_PARTICLES) {
                ...
                if (mc.level != null && mc.player != null) {
                    ...
                    // FIX FPS: Частицы почти убраны - только раз в 200 тиков (10 секунд) и только для ближайшего ядра
                    if (clientTickCounter % 200 == 0) {
                        java.util.List<NexusCoreEntity> cores = mc.level.getEntitiesOfClass(
                                NexusCoreEntity.class,
                                mc.player.getBoundingBox().inflate(32)); // Уменьшен радиус до 32 блоков
                        
                        // Обрабатываем только ближайшее ядро
                        NexusCoreEntity nearest = null;
                        double nearestDist = Double.MAX_VALUE;
                        for (NexusCoreEntity core : cores) {
                            if (core.isRemoved() || !core.isAlive()) continue;
                            double dist = mc.player.distanceToSqr(core);
                            if (dist < nearestDist) {
                                nearestDist = dist;
                                nearest = core;
                            }
                        ...
                        if (nearest != null) {
                            // Спавним только 1 частицу раз в 10 секунд
                            nearest.spawnRadiusParticles();
                            // Желтые частицы тоже очень редко
                            if (clientTickCounter % 400 == 0) {
                                nearest.spawnGlowParticles();
                            }
                        }
                    }
                }
